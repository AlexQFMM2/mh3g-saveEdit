...
武器详情右侧可以实时浏览游戏模型：四向箭头每次旋转 15°并支持长按；也可使用左键精细旋转、
右键平移、滚轮缩放，双击或“重置视角”恢复自动取景。完整整合包已经把 558 个武器 ARC 放在程序旁的固定目录，解压后即可使用，
...
的真实材质索引读取 MRL，分别绑定主贴图、Alpha 和环境贴图，并使用 MRL 材质常量控制颜色、
粗糙度与反射；在线性空间完成照明、曝光和 Gamma 转换。当前武器 ARC 没有独立法线或高光
TEX，相关材质槽已保留供后续防具等资源使用。游戏专用粒子、发光和动态机关仍使用静态近似。
